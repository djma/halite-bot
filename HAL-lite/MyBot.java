...
// v14f: fixed collision resolution that became buggy since v13
// v14g: fixed repulsion map that became buggy since v13
// v15: made expansion tolerable to 3 wait moves + a few small fixes
// v15b: optimized growth period.. again
...
// TODO: If used up half time, don't use expansion anymore
...
public class MyBot {
  ...
  private static void greedyExpandWithProdLoss() {
      ...
      if (hasValidCandidate) {
        ...
        Collections.sort(sortedCandidates, new Comparator<Location>() {
          public int compare(Location l1, Location l2) {
            ...
            double value1 = (double) ed1.twoStepProd / (ed1.twoStepStrength + ed1.prodLoss*3);
            double value2 = (double) ed2.twoStepProd / (ed2.twoStepStrength + ed2.prodLoss*3);
    ...
    } while (hasValidCandidate && (System.currentTimeMillis() - startTime < 900));
...
//  }
    ...
    while (!toVisitQueue.isEmpty()) {
      ...
      int tolerableWait = 3;
      if (toVisitQueue.isEmpty() && cur.targetStrg < curSite.strength + cur.prodLoss*tolerableWait) { // long term view hack
        for (Location visited : cur.visited) {
          finalVisited.add(visited);
        }
        move = new Move(cur.loc, Direction.STILL);
        prodLoss = cur.prodLoss + (candidateSite.production - 1)*tolerableWait;
        // penalize a little bit by adding to the two step strength?
        break;
      }
    }
...
//        }
          if (innerArea.containsKey(newLoc) &&
              ...
              newSite.strength >= 6*newSite.production) {
  ...
  private static void resolveEnsembleMap() {
    ...
    for (Location l : outerRing.keySet()) {
      if (repulsionMap.containsKey(l) && 
          outerRing.get(l).dir == Direction.reverseDirection(repulsionMap.get(l)) &&
          outerRing.containsKey(gameMap.getLocation(l, outerRing.get(l).dir)) &&
          outerRing.get(gameMap.getLocation(l, outerRing.get(l).dir)).isFighting
          ) {
        outerRing.put(l, new OuterRingMemory(Direction.STILL, outerRing.get(l).isFighting));
      }
    }
  }

  private static void battleFormation1() {
    for (Location l : noMansLand.keySet()) {
      for (Direction d : Direction.CARDINALS) {
        Location ld = gameMap.getLocation(l, d);
        Site ld_s = gameMap.getSite(ld);
        Location lvd = gameMap.getLocation(l, Direction.reverseDirection(d));
        Site lvd_s = gameMap.getSite(lvd);
        if (outerRing.containsKey(ld) && outerRing.get(ld).dir == Direction.reverseDirection(d) &&
            gameMap.getSite(lvd).owner != 0 &&
            gameMap.getSite(lvd).owner != myID) {
          Location ll = gameMap.getLocation(l, Direction.turnLeft(d));
          if (gameMap.getSite(ll).owner == 0 && gameMap.getSite(ll).strength == 0) {
            Location ldl = gameMap.getLocation(ld, Direction.turnLeft(d));
            Site ldl_s = gameMap.getSite(ldl);
            Location lvdl = gameMap.getLocation(lvd, Direction.turnLeft(d));
            Site lvdl_s = gameMap.getSite(lvdl);
            if (outerRing.containsKey(ldl) && outerRing.get(ldl).dir == Direction.reverseDirection(d) && lvdl_s.strength < 20) {
              if (ldl_s.strength + ld_s.strength > Math.min(lvd_s.strength, lvdl_s.strength)) {
                if (lvdl_s.strength > lvd_s.strength) {
                  outerRing.put(ld, new OuterRingMemory(Direction.STILL, true));
                }
                else {
                  outerRing.put(ldl, new OuterRingMemory(Direction.STILL, true));
                }
              }
            }
          }

          Location lr = gameMap.getLocation(l, Direction.turnLeft(d));
          if (gameMap.getSite(lr).owner == 0 && gameMap.getSite(lr).strength == 0 && outerRing.get(ld).dir == Direction.reverseDirection(d)) {
            Location ldr = gameMap.getLocation(ld, Direction.turnRight(d));
            Site ldr_s = gameMap.getSite(ldr);
            Location lvdr = gameMap.getLocation(lvd, Direction.turnRight(d));
            Site lvdr_s = gameMap.getSite(lvdr);
            if (outerRing.containsKey(ldr) && outerRing.get(ldr).dir == Direction.reverseDirection(d) && lvdr_s.strength < 20) {
              if (ldr_s.strength + ld_s.strength > Math.min(lvd_s.strength, lvdr_s.strength)) {
                if (lvdr_s.strength > lvd_s.strength) {
                  outerRing.put(ld, new OuterRingMemory(Direction.STILL, true));
                }
                else {
                  outerRing.put(ldr, new OuterRingMemory(Direction.STILL, true));
                }
              }
            }
          }
