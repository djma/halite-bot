...
public class FailedBot13 {
  ...
  static PrintWriter log;
    ...
    log = new PrintWriter("./log.txt", "UTF-8");
		...
		Networking.sendInit("FailedBot13");
      ...
      else if (innerArea.size() < 5) {
        aggressiveEarlyCombat();
        for (Location l : innerArea.keySet()) {
          outerRing.put(l, new OuterRingMemory(innerArea.get(l).dir, true));
        }
        resolveCollisions();
        for (Location l : innerArea.keySet()) {
          moves.add(new Move(l, innerArea.get(l).dir));
  ...
  private static double computeTwoStepBestProdOverStrCombat(Location l) {
    double bestProdOverStr = 0.000001;
    Site s = gameMap.getSite(l);
    for (Direction d : Direction.CARDINALS) {
      Site ss = gameMap.getSite(l, d);
      if (ss.owner != myID) {
        for (Direction dd : Direction.CARDINALS) {
          Site sss = gameMap.getSite(gameMap.getLocation(l, d), dd);
          if (dd != d && sss.owner != myID) {
            double prodOverStr = (double)(s.production + ss.production + sss.production) / 
                                         (s.strength + (ss.owner != 0 ? 0 : ss.strength) + (sss.owner != 0 ? 0 : sss.strength) + s.production + ss.production + 1);
            if (prodOverStr > bestProdOverStr) {
              bestProdOverStr = prodOverStr;
            }
          }
  ...
  private static void generateInnerAreaDiffusionMap() {
    ...
    for (Direction d : SHUFFLED_CARDINALS) {
      for (Location l : noMansLand.keySet()) {
        ...
        int newDist = 1 + s.strength / (s.production + 1);
  ...
  private static void aggressiveEarlyCombat() {
    for (Location l : outerRing.keySet()) {
      innerArea.put(l, new InnerAreaMemory(Direction.STILL, Integer.MAX_VALUE));
    }
    ...
    Deque<Location> toDiffuse = new LinkedList();
    for (Location l : noMansLand.keySet()) {
      toDiffuse.add(l);
    }
    while (!toDiffuse.isEmpty()) {
      Location current = toDiffuse.removeFirst();
      int currentDistance = innerArea.containsKey(current) ? innerArea.get(current).productionLossToOuterRing : 
                                   //outerRing.containsKey(current) && outerRing.get(current).isFighting ? -2 :
                                   Integer.MAX_VALUE;
      if (noMansLand.containsKey(current)) {
        currentDistance = (int)(1.0/computeTwoStepBestProdOverStrCombat(current));
      }
      for (Direction d : SHUFFLED_CARDINALS) {
        Location next = gameMap.getLocation(current, d);
        Site nextSite = gameMap.getSite(current, d);
        if (innerArea.containsKey(next) && innerArea.get(next).productionLossToOuterRing > currentDistance + 1) {
          toDiffuse.add(next);
          innerArea.put(next, new InnerAreaMemory(Direction.reverseDirection(d), 0, currentDistance + 1));
        }
      }
    }
    ...
    for (Location l : innerArea.keySet()) {
      Site s = gameMap.getSite(l);
      if ((s.strength < 5*s.production && !(outerRing.containsKey(l) && outerRing.get(l).isFighting)) || 
          s.strength < gameMap.getSite(l, innerArea.get(l).dir).strength) {
        innerArea.get(l).dir = Direction.STILL;
      }
    }
  }
