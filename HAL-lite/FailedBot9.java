...
public class FailedBot9 {
  ...
  static Map<Location, Integer> phantomDamageDealt = new HashMap();
  ...
  static PrintWriter log;
    ...
    log = new PrintWriter("./log.txt", "UTF-8");
		...
		Networking.sendInit("FailedBot9");
...
//    if (ownerTerritory.get(myID) > 0.5 * gameMap.height * gameMap.width / ownerTerritory.size()) {
//      ADULT_AGE = 4;
//    }
...
//}
  ...
  private static void outerRingStrategy() {
    for (Location l : outerRing.keySet()) {
      ...
      Direction phantomDirection = Direction.STILL;
      int highestPossibleEnemyDamage = 0;
      int highestPossiblePhantomDamage = 0;
      int lowestStr = 256;
      ...
      for (Direction d : Direction.CARDINALS) {
        ...
        int possiblePhantomDamage = 0;
        if (target.owner != myID && target.strength == 0) {
          for (Direction d2 : Direction.DIRECTIONS) {
            Location potentialEnemyLoc = gameMap.getLocation(targetLoc, d2);
            Site potentialEnemySite = gameMap.getSite(potentialEnemyLoc);
            if (potentialEnemySite.owner != myID && potentialEnemySite.owner != 0) {
              possibleEnemyDamage += Math.min(current.strength, potentialEnemySite.strength);
            }
            if (potentialEnemySite.owner != myID) {
              possiblePhantomDamage += Math.min(current.strength, 255 - (phantomDamageDealt.get(potentialEnemyLoc) != null ? phantomDamageDealt.get(potentialEnemyLoc) : 0));
            }
          }
          ...
          if (possiblePhantomDamage > highestPossiblePhantomDamage) {
            phantomDirection = d;
            highestPossiblePhantomDamage = possiblePhantomDamage;
          }
        }
      }

      Direction selectedDirection = Direction.STILL;
      boolean engagingEnemy = false;
      if (enemyDirection != Direction.STILL || phantomDirection != Direction.STILL) {
        if (highestPossibleEnemyDamage > 0.5*highestPossiblePhantomDamage) {
          selectedDirection = enemyDirection;
          engagedEnemy = true;
          engagingEnemy = true;
        }
        else {
          selectedDirection = phantomDirection;
        }
      } else {
        selectedDirection = prodRatioDirection;
      }

      Location selectedLocation = gameMap.getLocation(l, selectedDirection);
      for (Direction d : Direction.DIRECTIONS) {
        Location phantomDmgLoc = gameMap.getLocation(l, d);
        Site phantomDmgSite = gameMap.getSite(phantomDmgLoc);
        if (phantomDmgSite.owner != myID) {
          phantomDamageDealt.put(phantomDmgLoc, Math.min(255, (phantomDamageDealt.get(phantomDmgLoc) != null ? phantomDamageDealt.get(phantomDmgLoc) : 0) + current.strength));
        }
      }
      outerRing.put(l, new OuterRingMemory(selectedDirection, engagingEnemy));
    }
  }
      ...
      for (Location l : noMansLand.keySet()) {
        ...
        while (innerArea.containsKey(newLoc) || outerRing.containsKey(newLoc)) {
          ...
          if (innerArea.containsKey(newLoc) &&
              ...
              newSite.strength >= getAdultAge(newLoc)*newSite.production) {
  ...
  private static int getAdultAge(Location l) {
    return 5; //- (l.y % 2);
  }
