...
public class FailedBot9b {
  ...
  static PrintWriter log;
    ...
    log = new PrintWriter("./log.txt", "UTF-8");
		...
		Networking.sendInit("FailedBot9b");
...
//}
  ...
  private static void generateInnerAreaDiffusionMap() {
    ...
    // Production loss routing leads to a lot of clumps
    ...
    while (!toDiffuse.isEmpty()) {
      ...
      int currentProdLossToOuterRing = innerArea.containsKey(current) ? innerArea.get(current).productionLossToOuterRing : 
      ...
      for (Direction d : SHUFFLED_CARDINALS) {
        ...
        if (innerArea.containsKey(next) && innerArea.get(next).productionLossToOuterRing > currentProdLossToOuterRing + nextSite.production) {
          toDiffuse.add(next);
          innerArea.put(next, new InnerAreaMemory(Direction.reverseDirection(d), 0, currentProdLossToOuterRing + nextSite.production));
        }
      }
    }
