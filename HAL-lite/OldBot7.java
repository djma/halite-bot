...
//    if (ownerTerritory.get(myID) < 0.25*(gameMap.height * gameMap.width)/ownerTerritory.size()) {
...
//}
  ...
  private static void resolveCollisions() {
    ...
    for (Location l : noMansLand.keySet()) { 
      ...
      for (Direction d : Direction.CARDINALS) {
        ...
        if (outerRing.containsKey(checkloc) && outerRing.get(checkloc).dir == Direction.reverseDirection(d)) {
          if ((double)(Math.min(255, currentStrength + checksite.strength) - currentStrength) / checksite.strength > 0.8) {
    ...
    for (Location l : outerRing.keySet()) {
      Site s = gameMap.getSite(l);
      int currentStrength = outerRing.get(l).dir == Direction.STILL ? s.strength + s.production : 0;
      for (Direction d : Direction.CARDINALS) {
        Location checkloc = gameMap.getLocation(l, d);
        Site checksite = gameMap.getSite(checkloc);
    ...
    while (!toResolve.isEmpty()) {
      ...
      // Currently same logic as outerRing resolution.
      Site s = gameMap.getSite(l);
      int currentStrength = innerArea.get(l).dir == Direction.STILL ? s.strength + s.production : 0;
      for (Direction d : Direction.CARDINALS) {
        Location checkloc = gameMap.getLocation(l, d);
        Site checksite = gameMap.getSite(checkloc);
        if (innerArea.containsKey(checkloc) && innerArea.get(checkloc).dir == Direction.reverseDirection(d)) {
    ...
    for (Location l : innerArea.keySet()) {
      // Should be free of collisions already
    }
  }
